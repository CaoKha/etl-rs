...
fn replace_delimiters_inside_nom(
    nom: &str,
    pattern: &str,
    replacement: &str,
            ...
            (caps.get(0).map(|m| m.start()), caps.get(0).map(|m| m.end()))
        {
            let before_is_space = start > 0
                && nom
                    ...
                    .unwrap_or(false);
            let after_is_space = end < nom.len()
                && nom
                    .chars()
                    .nth(end)
                    .map(|c| c.is_whitespace())
...
fn transform_nom(text: Option<&str>) -> Option<String> {
    text.and_then(|nom| {
        let nom = nom.trim();
        if nom.is_empty() {
            return None;
        ...
        let nom = Regex::new(r"\-+").ok()?.replace_all(&nom, " ").to_string();
        let nom = Regex::new(r"\s+").ok()?.replace_all(&nom, " ").to_string();
...
pub fn transform_col_nom(series: &Series) -> PolarsResult<Option<Series>> {
    let opt_series = series
        .str()?
        .iter()
        .map(|text| transform_nom(text))
        .collect::<StringChunked>();
    Ok(Some(opt_series.into_series()))
}

...
fn partition_by_name(lf: LazyFrame) -> PolarsResult<HashMap<String, LazyFrame>> {
	...
	for name in unique_names {
		let filtered_lf = lf.clone().filter(col(Hdd::Nom.as_str()).eq(lit(name.clone())));
		frames_map.insert(name, filtered_lf);
	}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let mut df_test = transform_deduplication_optimize(lf.clone())?.collect()?;
	println!("Deduplication optimize test: {:#?}", df_test);
	...
	CsvWriter::new(&mut csv_file).finish(&mut df_test)?;
